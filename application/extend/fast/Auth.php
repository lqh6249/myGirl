 ...
 * 4，支持规则表达式。
 *      在think_auth_menu 表中定义一条规则，condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100
...
class Auth
{
    ...
    public function getRuleList($uid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_menu'])->where($where)->field('id,pid,condition,icon,name,title,ismenu')->select();
